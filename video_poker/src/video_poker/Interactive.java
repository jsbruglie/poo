...
public class Interactive extends Mode {
	...
	public void execute(Score score, Player player, Deck deck, Statistics stats){
		...
		while(true){
			...
			if(phase == Phase.Deal){
				//Deal
				String read = reader.nextLine();
				dealState(read, deck, player, stats);
			...
			if(phase == Phase.Hold){
				//Hold
				String read = reader.nextLine();
				holdState(read, deck, player, stats);
	...
	private void betState(String read, Player player, Statistics stats){
		...
		if(tokens[0].equals("b")){
			...
			//Player wants to see their credit
	...
	private void dealState(String read, Deck deck, Player player, Statistics stats){
		if(read.equals("d")){
	...
	private void holdState(String read, Deck deck, Player player, Statistics stats){
		String[] tokens = read.split(" ");
